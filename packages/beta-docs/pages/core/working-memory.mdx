...
`WorkingMemory` is manipulated through list-like functions (`slice`, `map`, etc) and `CognitiveStep`s which use a model to modify memory and return values.
...
// easily add a memory (returning a new WorkingMemory with the appended memory)
...
import { createCognitiveStep } from "@opensouls/engine"
...
// alternatively
const [newMemory, stream, brainstormIdeasPromise] = await brainstorm(memory, "Think of 3 butterflies", { stream: true })
